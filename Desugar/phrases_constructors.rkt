...
(struct stat-fun-call (prefixexp args))

(struct stat-method-call (prefixexp method-name args))
...
; TODO: no debería utilizar esta construcción
(struct do-end (stats))
...
(define (concrete-grammar-s phrase)
  (match phrase
    ...
    ((return e) (append (term (return))
                        (concrete-grammar-exps e)))

    ; TODO: no tendría que tener esta construcción
    ...
    ((local-vars ids rvalues scope) (append (list 'local)
    ...
    ((var-assign lvalues rvalues) (append (concrete-grammar-exps lvalues)
                                          (list '=)
                                          (concrete-grammar-exps rvalues)))
...
(define (concrete-grammar-e phrase)
  (match phrase
    ...
    ; function definition
    ((func-def label formal-params body)
     (append (term (function ))
...
(define (concrete-grammar-functioncall phrase)
  (match phrase
    ...
    ((method-call prefixexp method-name args)
     (list (concrete-grammar-e prefixexp)
           (term :)
...
(define (concrete-grammar-form-params form-params)
  (match form-params
    ...
    ((vararg-params ids)
     (list (append (concrete-grammar-exps ids)
...
(define (concrete-grammar-conc-stats phrase)
  (match phrase
    ...
    ((conc-stats stats)
     (append (list (concrete-grammar-s (list-ref stats 0)))
...
(define (concrete-grammar-tablefields flds)
  (match flds
    ...
    ((fields l)
     (append (list (concrete-grammar-tablefield (list-ref l 0)))
